...
def data_preprocess(data_index, size = 18, step = 1):
    ...
    for ch, cp, ca, atoms in [('m', 'g', 0.5, pro_zip), ('r', 'b', 1.0, lig_zip)]:
        ...
        for atom in atoms:
            ...
            and abs(atom[2] - center[2]) < size * step / 2):
                ...
                voxel[x1, y1, z1, channel] += abs(atom_recenter[0]/step-x2)*abs(atom_recenter[1]/step-y2)*abs(atom_recenter[2]/step-z2)*atom_recenter[3]
                voxel[x1, y1, z2, channel] += abs(atom_recenter[0] / step - x2) * abs(atom_recenter[1] / step - y2) * abs(
                    atom_recenter[2] / step - z1)*atom_recenter[3]
                voxel[x1, y2, z1, channel] += abs(atom_recenter[0] / step - x2) * abs(atom_recenter[1] / step - y1) * abs(
                    atom_recenter[2] / step - z2)*atom_recenter[3]
                voxel[x2, y1, z1, channel] += abs(atom_recenter[0] / step - x1) * abs(atom_recenter[1] / step - y2) * abs(
                    atom_recenter[2] / step - z2)*atom_recenter[3]
                voxel[x1, y2, z2, channel] += abs(atom_recenter[0] / step - x2) * abs(atom_recenter[1] / step - y1) * abs(
                    atom_recenter[2] / step - z1)*atom_recenter[3]
                voxel[x2, y2, z1, channel] += abs(atom_recenter[0] / step - x1) * abs(atom_recenter[1] / step - y1) * abs(
                    atom_recenter[2] / step - z2)*atom_recenter[3]
                voxel[x2, y1, z2, channel] += abs(atom_recenter[0] / step - x1) * abs(atom_recenter[1] / step - y2) * abs(
                    atom_recenter[2] / step - z1)*atom_recenter[3]
                voxel[x2, y2, z2, channel] += abs(atom_recenter[0] / step - x1) * abs(atom_recenter[1] / step - y1) * abs(
                    atom_recenter[2] / step - z1)*atom_recenter[3]
